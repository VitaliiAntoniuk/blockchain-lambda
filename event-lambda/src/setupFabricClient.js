...
async function setupClient() {
    ...
    const username = "lambdaUser";
    const privatePEM = await getSecret("pk", username);
    const signedPEM = await getSecret("signcert", username);

	const fabricUser = await fabric_client.createUser({username, mspid: config.mspID, cryptoContent: {privateKeyPEM: privatePEM, signedCertPEM: signedPEM}, skipPersistence: true});
